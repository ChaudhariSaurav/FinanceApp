            ...
            <Box key={index} borderWidth={1} borderRadius="lg" overflow="hidden" boxShadow="md">
              <Box p={4} bg="gray.50">
                <HStack justifyContent="space-between">
                  ...
                  <Tooltip label={url.toLowerCase().endsWith('.pdf') ? "Download PDF" : "View Image"}>
                    <IconButton
                      icon={url.toLowerCase().endsWith('.pdf') ? <LuDownload /> : <LuImage />}
                      onClick={() => url.toLowerCase().endsWith('.pdf') ? handleDownload(url, `document_${index + 1}.pdf`) : window.open(url, '_blank')}
                      aria-label={url.toLowerCase().endsWith('.pdf') ? "Download PDF" : "View Image"}
                      size="sm"
                    />
                  </Tooltip>
                </HStack>
              ...
              {url.toLowerCase().endsWith('.pdf') ? (
                <Box p={4} bg="gray.100" textAlign="center" m={5} boxShadow={'xl'}>
                  <LuFileText size={48} />
                  <Text mt={2}>Document {index + 1}</Text>
                </Box>
              ) : (
                <Image src={url} alt={`Uploaded document ${index + 1}`} objectFit="cover" height="200px" width="100%" />
              )}
            </Box>
          ))}
        </SimpleGrid>
    ...
    <AppLayout>
      <Container maxW="container.xl" py={8}>
        <VStack spacing={8} align="stretch">
          <Heading size="lg" textAlign="center">Uploaded Documents</Heading>
          ...
          {!customerDocs && !guarantorDocs && (
            <Box textAlign="center" p={8} bg="gray.50" borderRadius="md"  transition="all 0.3s"
            _hover={{
              shadow: "md",
